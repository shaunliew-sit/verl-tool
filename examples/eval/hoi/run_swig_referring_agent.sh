#!/bin/bash
# Run SWIG Referring Evaluation with HOI Agent
#
# Prerequisites:
...
#
# Usage:
#   bash examples/eval/hoi/run_swig_referring_agent.sh
#
...
#   BERTSCORE_GPU   - GPU for BERTScore (default: 4)
...
#   OUTPUT_DIR      - Output directory (default: results/swig_referring_agent)
...
BERTSCORE_GPU="${BERTSCORE_GPU:-4}"
VERBOSE="${VERBOSE:-true}"
SAVE_VIZ="${SAVE_VIZ:-true}"
UNIQUE_RUN="${UNIQUE_RUN:-true}"
WANDB="${WANDB:-true}"
OUTPUT_DIR="${OUTPUT_DIR:-results/swig_referring_agent}"
...
LOG_FILE="${LOG_DIR}/eval-swig-referring-${TIMESTAMP}.log"
...
echo "BERTScore GPU:  $BERTSCORE_GPU"
